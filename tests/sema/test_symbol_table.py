...
def test_SymbolTable_func_template_spec0():
    ...
    foo.template_params = (T,)

    table = SymbolTable()
    table.insert(FuncSymbol("foo"), foo)

    call = ast.Call(func=ast.UFunction(name="foo", loc=None),
...
def test_SymbolTable_func_template_spec1():
    ...
    call = ast.Call(func=ast.UFunction(name="foo", loc=None),
                    ...
                    args=(ast.CallParam(name="a", value=ast.make_const(1, loc=None), loc=None),
                          ast.CallParam(name="b", value=ast.make_const(1.0, loc=None), loc=None)),
                    loc=None)
    funcs = table.lookup(FuncSymbol("foo"))
    assert funcs
